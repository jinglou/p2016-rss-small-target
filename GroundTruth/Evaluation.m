...
%
% Notes:
%	1. To use this code for evaluation, the pixel-level resulting images (PNG format) should be provided. 
% 	You can create a new subfolder in each dataset folder and put your detection results in the subfolder, 
% 	then add your model name to the cell "Models" of this code.
% 	We provide two example subfolders ([MSER] and [RSS]) in the dataset folders for reference.
% 
% 	2. To compute the Precision score for each image, the number of all detected targets should be provided. 
% 	This code use the "regionprops" function of Image Processing Toolbox to automatically calculate it from the binary mask image.
% 
% 	3. This code generates the following evaluation results:
% 		a) In each dataset folder, the resulting "PRF_XXXX.mat" file includes the values of Precision, Recall and F-measure 
% 		for each image.
% 		b) The resulting "Stat.mat" file includes the average values of Precision, Recall and F-measure for each model. 
%
%
